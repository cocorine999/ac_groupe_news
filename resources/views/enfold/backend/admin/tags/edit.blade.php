...
@section('title','Modification d\'un tag : '.$tag->name)
@push('css')
                    ...
                    <form method="POST" action="{{ route('admin.tags.update',$tag->id) }}">
                        ...
                        <div class="form-group">
                            <div class="form-line col-">
                                <input type="text" id="name" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ $tag->name }}" required autocomplete="name" autofocus placeholder="Entrez le nom du tag">
                                @error('name')
